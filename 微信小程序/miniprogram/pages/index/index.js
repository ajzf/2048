...
Page({
  ...
  //开始游戏
  initGame: function () {
    //将更新最高数据放在这，先判断此时hscore是否等于0，不等于就上传，等于就过
    //hdata = 0
    if(hdata < htemp)hdata = htemp;
    console.log('htemp is '+htemp)
    console.log('start is '+hdata)
    db.collection('2048score').where({
      _openid: 'user-open-id'
    })
    .get({
      success: function(res) {
        hdata = res.data[0].hscore
      }
    })
   
    console.log('now is '+hdata)
    console.log(this.data.score)
    if(this.data.score > htemp){
      htemp = this.data.score;
    }
    if(this.data.score > hdata){
      hdata = this.data.score;
      db.collection('2048score').where({
        _openid: 'user-open-id'
      })
      .update({
        ...
        success: function(res) {
         console.log('hell00000')
        }
      })
    }
    console.log('then is '+hdata)
      this.setData({
        // boxes: initBoxes,
        boxes : [
         [0, 0, 0, 0],
